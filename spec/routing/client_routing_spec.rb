...
RSpec.describe Api::V1::ClientsController, type: :routing do
  describe 'routing' do
    ...
    it { should route(:post, 'api/v1/client/name').to(action: :find_name) }

    it { should route(:post, 'api/v1/client/create').to(action: :create) }

    it { should route(:patch, 'api/v1/client/1').to(action: :update, id:1) }
